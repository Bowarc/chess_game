...
impl super::TElement for TextEdit{
    fn draw(
        ...
        let text = ggez::graphics::Text::new(ggez::graphics::TextFragment::new(txt).scale(self.font_size as f32));

        render_request.add(text, crate::render::DrawParam::default().rect(rect), crate::render::Layer::Ui);
        
        self.size = ggez::mint::Point2::from_slice(&[self.width.clone(), crate::ui::Value::from(self.font_size * self.rows as f64)]);
        Ok(())
    }
    ...
    fn on_text_input(
        ...
        match character{
            ...
            '\u{005B}'..='\u{0060}' | /* [\]^_` */ 
            '\u{0061}'..='\u{007A}' | /* abcdefghijklmnopqrstuvwxy */ 
            '\u{007B}'..='\u{007E}'   /* {|}~ */ => {
