...
impl Connected {
    ...
    fn update_client(mut self) -> Result<Self, super::State>{
        ...
        if let Some(game_id) = received_game_id{
            return Err(
                super::Playing::new(self.client, game_id).into()
            );
        }
        ...
        if let Some(active_games) = self.active_games.inner(){
            for game in active_games.iter(){
                ...
                if el.clicked_this_frame(){
                    ...
                    self.client.send(shared::message::ClientMessage::GameJoinRequest(game.id())).unwrap();
                }
            }

            if let Some(el) = self.ui.try_get_element("Game_crate_button").and_then(|el|el.try_inner_mut::<crate::ui::element::Button>()){
                if el.clicked_this_frame(){
                    debug!("I wanna create a new game");
                    self.client.send(shared::message::ClientMessage::GameCreateRequest).unwrap();
...
fn create_games_ui(ui_mgr: &mut crate::ui::UiManager, games: &[shared::game::Game]) {
    ...
    for (i, game) in games.iter().enumerate() {
        ...
        ui_mgr.add_element(
            ui::element::Element::new_text(
                ...
                vec![ui::element::TextBit::new_text(
                    "Join".to_string(), Some(render::Color::random_rgb())
                )]
    ...
    ui_mgr.add_element(
        ui::element::Element::new_text(
            ...
            new_b_size.x() * 0.1,
            ui::Style::new(render::Color::default(), None, None),
            vec![ui::element::TextBit::new_text("Create new", Some(render::Color::random_rgb()))],
        ),
        group_name,
    );
}
